...
class ManageCourseDescription extends React.Component {
        ...
        $.ajax({
            ...
            success: (data, status) => {
                if (data.code == 1001) {
                    ...
                    this.props.dispatch(setCoursePreviewVideo(data.previewvideo, this.props.params.id))
    ...
    render() {
        ...
        if (this.props.course_previewvideo) {
            video = <div>
                <video id={"video-" + this.props.params.id} className="video-js center-block" controls preload="auto"
                    width="640" height="auto">
                    <source src={this.props.course_previewvideo}
                        type='video/mp4' />
                </video>
                <br />
            </div>
        ...
        let genre = _.find(this.props.genreList, { _id: this.state.course_category })
        let subgenreList
        if (genre) {
        ...
        return (
            <form onSubmit={this.onSubmit.bind(this)}>
                <div className="managecourse-title-box">
                    <h1><em>Course Description</em></h1>
                </div>
                <div className="managecourse-content">
                    <div className="item">
                        <label>Course Title</label>
                        <input type="text" required className="form-control input-lg"
                            placeholder="Course Title"
                            value={this.state.course_name} onChange={this.handleCourseName.bind(this)} />
                    </div>
                    <div className="item">
                        <label>Course Description</label>
                        <textarea className="form-control" rows="10" id="ckeditor"></textarea>
                    </div>
                    <div className="item">
                        <label>Course Image: </label>
                        <img className="center-block image-description" src={this.state.course_image} />
                        <div className="input-group">
                            <label className="input-group-btn">
                                <span className="btn btn-primary glyphicon glyphicon-picture" style={{ top: 0 }}>
                                    <input type="file" onChange={this.handleImageFile.bind(this)}
                                        style={{ display: 'none', multiple: '' }} />
                                </span>
                            </label>
                            <input type="text" className="form-control" readOnly={true} value={this.state.file.name || ''} />
                        </div>
                    </div>
                    <div className="item">
                        {video}
                        <div className="form-group">
                            ...
                            <div className="input-group">
                                ...
                                <input type="text" className="form-control" readOnly={true} value={this.state.previewvideoname || 'None'} />
                            </div>
                        </div>
                        <div className={'progress' + (this.state.progress_video ? '' : ' hide')}>
                    ...
                    <div className="item row">
                        <div className="col-xs-6 col-md-4">
                            ...
                            <select className="form-control" value={this.state.course_level} onChange={this.handleLevel.bind(this)}>
                        ...
                        <div className="col-xs-6 col-md-4">
                            ...
                            <select required className="form-control" value={this.state.course_category} onChange={this.handleCategory.bind(this)}>
                        ...
                        <div className="col-xs-6 col-md-4">
                            ...
                            <select required className="form-control" value={this.state.course_subcategory} onChange={this.handleSubcategory.bind(this)}>
                                <option>--Select Category--</option>
                                {subgenreList}
                            </select>
                        </div>
