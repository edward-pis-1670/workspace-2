...
import { Breadcrumb, Glyphicon, Button, Row, Col, Pager, ProgressBar } from 'react-bootstrap'
...
class Learning extends React.Component {
    ...
    onClickPrev(e) {
        ...
        let url = '/mycourses/learning' + '?page=' + (query.page - 1)
            + (query.name ? ('&name=' + query.name) : '') + (query.level ? ('&level=' + query.level) : '') + (query.free ? ('&free=' + query.free) : '')
    ...
    onClickNext(e) {
        ...
        let url = '/mycourses/learning' + '?page=' + (parseInt(query.page) + 1)
            + (query.name ? ('&name=' + query.name) : '') + (query.level ? ('&level=' + query.level) : '') + (query.free ? ('&free=' + query.free) : '')
            + (query.sort ? ('&sort=' + query.sort) : '')
        browserHistory.push(url)
    }
    onChangeFilterLevel(e) {
        ...
        let url = '/mycourses/learning' + '?page=' + query.page
        ...
        return <div>
            <div className='genre-info-box' style={{ backgroundColor: this.state.headerColor }} >
                <div className='container'>
                    ...
                    <p className='genre-title'>Learning</p>
                    <ul className="mycourses-navbar">
                        <li><Link activeClassName="active"
                            onlyActiveOnIndex={true} to='/mycourses/learning'>Learning</Link></li>
