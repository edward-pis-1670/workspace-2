...
import { setUser, showModal, setGetMyCourses, markAllAsRead, markRead } from '../actions'
...
class Navbar extends React.Component {
    ...
    render() {
        ...
        let notiCount = this.props.notis && this.props.notis.length > 0 ? this.props.notis.map(o => o.seen ? 0 : 1).reduce((a, b) => a + b, 0) : 0
        if (this.props.username == '') {
            navbarRight = <ul className="nav navbar-nav navbar-right">
                <li><a onClick={this.showFormLogin.bind(this)}><span className='glyphicon glyphicon-log-in'></span>{' '}Login</a></li>
                <li><a onClick={this.showFormSignup.bind(this)}><span className='glyphicon glyphicon-registration-mark'></span>{' '}Sign Up</a></li>
            </ul>
            formLogin = <FormLogin ref="formLogin"
                onClickSignup={this.showFormSignup.bind(this)}
                onClickForgotPassword={this.showFormForgotPassword.bind(this)} />
            formSignup = <FormSignup ref="formSignup"
                onClickLogin={this.showFormLogin.bind(this)} />
            formForgotPassword = <FormForgotPassword ref="formForgotPassword"
                onClickLogin={this.showFormLogin.bind(this)} />
        } else {
            navbarRight = <ul className="nav navbar-nav navbar-right">
                <li><Link to="/instructor"><span className='glyphicon glyphicon-pencil'></span>{' '}Instructor</Link></li>
                <li><Link to="/mycourses/learning"><span className='glyphicon glyphicon-list-alt'></span>{' '}Learning</Link></li>
                <li><Link><Glyphicon glyph='bell' />
                    {notiCount > 0 ?
                    ...
                    <div className='noti-dropdown'>
                        ...
                        <div className='noti-footer'>
                            <Glyphicon glyph='eye-open' /><Link to='/notifications'>{' '}See all</Link>
                        </div>
                    </div>
                </li>
                <li className="dropdown">
                    ...
                    <ul className="dropdown-menu">
                        {this.props.isAdmin ?
                            <li>
                                <Link to="/admin/users">
                                    <span className='glyphicon glyphicon-king'></span>{' '}Admin Page
                                </Link>
                            </li> : ''
                        }
                        <li><Link to="/user/edit-profile"><span className='glyphicon glyphicon-user'></span>{' '}My Profile</Link></li>
                        <li><Link to="/mycourses/wishlist"><span className='glyphicon glyphicon-heart-empty'></span>{' '}My Wishlist</Link></li>
                        <li className="divider"></li>
                        <li><a onClick={this.onClickLogout.bind(this)}><span className='glyphicon glyphicon-log-out'></span>{' '}Logout</a></li>
                    </ul>
                </li>
            </ul>
        }
                ...
                <nav className="navbar-main navbar navbar-default" role="navigation">
                    <div className="container-fluid">
                        <div className="navbar-header">
                            ...
                            <Link to="/courses" className="navbar-brand center-block">Academy</Link>
                        </div>
