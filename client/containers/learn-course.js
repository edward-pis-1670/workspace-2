...
class LearnCourse extends React.Component {
    ...
    render() {
        if (!this.props.course || this.props.course.lectures.length == 0) return <div></div>
        return (
            <div>
                ...
                <div id='wrapper'>
                    ...
                    <div id='page-content-wrapper'>
                        <video id="video" className="video-js" controls preload="false"
                            width="100%" height="auto">
                            <source src={'/api/resource/play-video-learning/' + this.props.params.id + '/'
                                + this.props.course.lectures[this.state.currentLectureIndex]._id} type='video/mp4' />
                        </video>
                    </div>
                </div>
                <div className='container' style={{ marginTop: '30px' }}>
                    <div className="panel panel-primary">
                        ...
                        <div className="panel-body">
                            <div className='row'>
                                <Col xs={3} md={2} style={{ paddingLeft: '0px', paddingRight: '0px' }}>
                                    <span>Avegare Rating</span>
                                    <div style={{ fontSize: '96px', fontWeight: 600, color: '#17aa1c', lineHeight: 1 }}>{this.props.course.star ? this.props.course.star.toFixed(1) : 0}</div>
                                    <div>
                                        <span className="course-rate">
                                            <span style={{ width: (this.props.course.star || 0) * 20 + '%' }}></span>
                                        </span>
                                    </div>
                                    <span style={{ display: 'block' }}><strong>Details({this.props.course.numberofreviews + ' '}reivews)</strong></span>
                                        ...
                                        this.state.reviews.map((review, index) => {
                                            return (
                                                <div className='info-rate' key={index}>
                                                    <img className="avatar-rate" src={'/api/resource/images?src=' + review.user.photo + '&w=50&h=50'} />
                                                    <div style={{ display: 'inline-block', verticalAlign: 'middle' }}>
                                                        <Link to={'/view-user/' + review.user._id} className="username-rate">{review.user.username}</Link>
                                                        <Glyphicon style={{ marginLeft: '10px' }} glyph='calendar' />
                                                    ...
                                                    <span dangerouslySetInnerHTML={{ __html: review.content.replace(/\n/g, '<br/>') }} className='content-rate'></span>
