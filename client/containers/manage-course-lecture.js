...
import {
    setUser, setGetMyCourses, setCourseLectures, addLecture,
    deleteLecture, setLectureVideo, setLectureName, setLecturePreview
} from '../actions'
    ...
    render() {
        return <Modal show={this.state.showModal}
            ...
            <Modal.Body style={{ overflow: 'auto' }}>
                ...
                <form className="form-horizontal" onSubmit={this.props.onSubmitAddLecture.bind(this)}>
                    ...
                    <div className="form-group">
                        <div className="col-sm-offset-1 col-sm-10">
                            <button disabled={this.state.isSubmitting} type="submit" className="btn btn-success btn-lg btn-block">ADD</button>
                        </div>
                    </div>
                </form>
            </Modal.Body>
        ...
        $.ajax({
            ...
            success: (data, status) => {
                ...
                if (data.code == 1001) {
                    ...
                    this.props.dispatch(setLectureVideo(data.lecture, this.props.params.id))
    ...
    render() {
        ...
        if (this.props.lectures) {
            lectures = this.props.lectures.map((lecture, index) => {
                ...
                if (lecture.video) {
                    video = <div>
                        <video id={"video-" + lecture._id} className="video-js center-block" controls preload="auto"
                            width="640" height="auto">
                            <source src={'/api/resource/play-video-lecturer/' + lecture._id + '?path=' + lecture.video} type='video/mp4' />
                        </video>
                        <br />
                    </div>
                }
                return (
                    <div className="item box white" key={index}>
                        ...
                        <div className={'progress' + (this.state.progress_list[lecture._id] ? '' : ' hide')}>
        ...
        return (
            <div>
                <div className="managecourse-title-box">
                    <h1><em>Lectures</em></h1>
                </div>
                <div className="managecourse-content">
                    {lectures}
                </div>
                <button className="btn btn-danger btn-lg btn-block" onClick={this.showModal.bind(this)}>
                    <span className="glyphicon glyphicon-plus-sign"></span>
                    {' '}Add Lecture</button>
                <ModalAddLecture ref="modalAddLecture" onSubmitAddLecture={this.onSubmitAddLecture.bind(this)} />
            </div>
        )
    }
}
