...
class ModalShowLectures extends Component {
    ...
    render() {
        return <Modal
            ...
            <Modal.Content>
                {this.state.course.lectures ? this.state.course.lectures.map((lecture, index) => {
                    return <div key={index}>
                        <strong>Lecture {index + 1}: {lecture.name}</strong>
                        <video id={'video-' + lecture._id} controls preload="false"
                            width="100%" height="auto">
                            <source src={'/api/resource/play-video-admin/' + lecture._id} type='video/mp4' />
                        </video>
...
class AdminReviewCourse extends Component {
    constructor(props) {
        super(props)
        this.state = {
            searchQuery: '',
    ...
    onClickPrev(e) {
        ...
        let url = '/admin/review-courses' + '?page=' + (query.page - 1)
    ...
    onClickNext(e) {
        ...
        let url = '/admin/review-courses' + '?page=' + (parseInt(query.page) + 1)
            + (query.searchQuery ? ('&searchQuery=' + query.searchQuery) : '')
            + (query.sort ? ('&sort=' + query.sort) : '')
        browserHistory.push(url)
    }
    onChangeFilterSort(e) {
        ...
        let url = '/admin/review-courses' + '?page=' + query.page
            + (query.searchQuery ? ('&searchQuery=' + query.searchQuery) : '')
            + '&sort=' + e.target.value
        browserHistory.push(url)
    }
                        ...
                        return <Table.Row key={index}>
                            ...
                            <Table.Cell>
                                {course.genre ?
                                    <div>
                                        <strong>Category :{' '}
                                            <a target='_blank' href={'/courses/' + course.genre._id}>{course.genre.name}</a>
                                            {' '}/{' '}
                                            <a target='_blank' href={'/courses/' + course.genre._id + '/' + course.subgenre._id}>{course.subgenre.name}</a>
                                        </strong>
                                        <br />
                                    </div> : ''
                                }
                                ...
                                <div>
                                    <span className="course-rate">
                                        <span style={{ width: (course.star || 0) * 20 + '%' }}></span>
                                    </span>
                                    <span style={{ fontWeight: 'bold' }} >
                                        {' ' + (course.star ? course.star.toFixed(1) : 0) + '(' + course.numberofreviews +
                                            ' ratings) • ' + course.numberofstudent + ' students enrolled'}
                                    </span>
                                </div>
                                <hr />
                            ...
                            <Table.Cell>
                                <strong style={{ color: '#5bc0de' }}>What are the requirements?</strong>
                                <ul>
                                    {course.needtoknow.map((item, index) => {
                                        return <li key={index}>{item}</li>
                                    })}
                                </ul>
                                <strong style={{ color: '#5bc0de' }}>What am I going to get from this course?</strong>
                                <ul>
                                    {course.willableto.map((item, index) => {
                                        return <li key={index}>{item}</li>
                                    })}
                                </ul>
                                <strong style={{ color: '#5bc0de' }}>What is the target students?</strong>
                                <ul>
                            ...
                            <Table.Cell>
                                <a target="_blank" href={'/view-user/' + course.lecturer._id}>{course.lecturer.username + ''}</a>
                            </Table.Cell>
                            <Table.Cell textAlign='center'>
                                <Button positive onClick={e => this.viewCourse(e, course)}><Glyphicon glyph='eye-open' /></Button>
                                <hr />
                                <Button.Group size='mini'>
                                    <Button positive onClick={e => this.acceptCourse(e, course._id)}>Accept</Button>
                                    <Button.Or />
                                    <Button negative onClick={e => this.refuseCourse(e, course._id)}>Refuse</Button>
                                </Button.Group>
                            </Table.Cell>
                        </Table.Row>
                    })}
...
AdminReviewCourse = connect()(AdminReviewCourse)

export default AdminReviewCourse