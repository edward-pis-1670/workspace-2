...
class ViewUser extends React.Component {
  ...
  componentDidMount() {
    ...
    $.post(
      API_URL + "/users/view-user",
      {
        id: Number(this.props.params.id),
      },
      (data, status) => {
        if (data.code == 200) this.setState({ user: data.user });
      }
    );
  }
  render() {
    if (!this.state.user._id) return <div></div>;
    let courses = this.state.user.mycourses.slice(
      8 * this.state.page - 8,
      8 * this.state.page
    );
    return (
            ...
            <div className="panel-body">
              <div className="row">
                ...
                <div className="col-xs-10 col-md-11">
                  <Link
                    to={"/view-user/" + this.state.user._id}
                  ...
                  {this.state.user.youtube ? (
                    <a
                      target="_blank"
                      href={
                        "https://www.youtube.com/" + this.state.user.youtube
                      }
                    >
                      <img
                        className="icon-social"
                  ...
                  {this.state.user.linkedin ? (
                    <a
                      target="_blank"
                      href={
                        "https://www.linkedin.com/" + this.state.user.linkedin
                      }
                    >
                      <img
                        className="icon-social"
                  ...
                  {this.state.user.googleid ? (
                    <a
                      target="_blank"
                      href={
                        "https://plus.google.com/" + this.state.user.googleid
                      }
                    >
                      <img
                        className="icon-social"
                  ...
                  {this.state.user.facebookid ? (
                    <a
                      target="_blank"
                      href={
                        "https://facebook.com/" + this.state.user.facebookid
                      }
                    >
                      <img
                        className="icon-social"
              ...
              <div
                dangerouslySetInnerHTML={{ __html: this.state.user.biography }}
