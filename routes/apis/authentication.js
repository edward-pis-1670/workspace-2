...
const FacebookStrategy = require("passport-facebook").Strategy,
  ...
  FACEBOOK_APP_ID = "236793534545738",
  // FACEBOOK_APP_SECRET = "8f41a1dba35280da33b668ecc713348d";
  FACEBOOK_APP_SECRET = "7a68d067594727d0289806537b6f9dd8";
...
passport.deserializeUser(function (id, done) {
  ...
  User.findOne({ _id: id })
    .select({ password: 0, __v: 0, updatedAt: 0, createdAt: 0, mycourses: 0 })
    .exec((err, data) => {
      done(err, data);
    });
});
      ...
      User.findByGgid(profile.id, (err, user) => {
        ...
        if (!user) {
          ...
          User.findWithEmail(email, (err, user) => {
            ...
            if (!user) {
              if (profile.displayName == "") return done(err);
...
router.post("/forgotpassword", (req, res, next) => {
  ...
  bcrypt.hash(newPassword, 10, function (err, hash) {
    ...
    User.findOneAndUpdate(
      ...
      if (err || !data) return res.send({ code: 404, message: "error" });
      let mainOptions = {
        // thiết lập đối tượng, nội dung gửi mail
        from: "Academy",
        to: req.body.email,
